...
def fetch_prompt(prompt_id):
    try:
        return struct.ai.get_prompt(prompt_id)
    except Exception:
        wiz.response.status(404, message="프롬프트 템플릿을 찾을 수 없습니다.")
...
if request_path == "/api/admin/ai/prompts":
    if method == "GET":
        admin_prompt_list()
    if method == "POST":
        admin_prompt_create()
...
segment = wiz.request.match("/api/admin/ai/prompts/<prompt_id>/activate")
if segment is not None:
    before = fetch_prompt(segment.prompt_id)
    if method != "PUT":
        wiz.response.status(405, message="Method not allowed")
    try:
        data = request_data()
        reason = data.get("changeReason") or data.get("change_reason") or ""
        struct.ai.activate_prompt(segment.prompt_id, admin_user_id=struct.session_user_id(), change_reason=reason)
        prompt = fetch_prompt(segment.prompt_id)
        struct.admin_log.create("ai_prompt_activate", "ai_prompt_template", segment.prompt_id, before, prompt)
    except Exception as error:
        wiz.response.status(400, message=str(error))
    wiz.response.status(200, prompt=prompt_dto(prompt))

segment = wiz.request.match("/api/admin/ai/prompts/<prompt_id>/deactivate")
if segment is not None:
    before = fetch_prompt(segment.prompt_id)
    if method != "PUT":
        wiz.response.status(405, message="Method not allowed")
    try:
        data = request_data()
        reason = data.get("changeReason") or data.get("change_reason") or ""
        struct.ai.deactivate_prompt(segment.prompt_id, admin_user_id=struct.session_user_id(), change_reason=reason)
        prompt = fetch_prompt(segment.prompt_id)
        struct.admin_log.create("ai_prompt_deactivate", "ai_prompt_template", segment.prompt_id, before, prompt)
    except Exception as error:
        wiz.response.status(400, message=str(error))
    wiz.response.status(200, prompt=prompt_dto(prompt))

segment = wiz.request.match("/api/admin/ai/prompts/<prompt_id>")
if segment is not None:
    before = fetch_prompt(segment.prompt_id)

    if method == "GET":
        wiz.response.status(200, prompt=prompt_dto(before))

    if method == "PUT":
        try:
            data = request_data()
            payload = prompt_payload(data)
            struct.ai.update_prompt(segment.prompt_id, payload, admin_user_id=struct.session_user_id())
            prompt = fetch_prompt(segment.prompt_id)
            struct.admin_log.create("ai_prompt_update", "ai_prompt_template", segment.prompt_id, before, prompt)
        except Exception as error:
            wiz.response.status(400, message=str(error))
        wiz.response.status(200, prompt=prompt_dto(prompt))
...
wiz.response.status(404, message="지원하지 않는 프롬프트 관리 API 경로입니다.")
